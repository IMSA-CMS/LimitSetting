...
RooPDF_DSCB create_signal_pdf(std::string channel_name, RooRealVar& mass, RooRealVar& realHiggsMass, RooRealVar& branch_1, RooRealVar& branch_2);
...
void construct_models_Higgs()
{
	...
	// RooPDF_DSCB is custom ; it was made using RooClassFactory::makePdf("RooPDF_DSCB"....)
	...
	std::vector<std::vector<RooPDF_DSCB>> signal_pdfs;
	...
	{
		std::vector<RooPDF_DSCB> signal_X_and_Y;
		...
		{
			RooPDF_DSCB signal_pdf(create_signal_pdf(channel, mass, realHiggsMass, Bee, Beu), (channel + "_signal_" + X_or_Y).c_str());
