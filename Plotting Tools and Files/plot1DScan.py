...
def read(scan, param, files, ycut):
    ...
    plot.RemoveGraphYAbove(graph, 500)
    # graph.Print()
    return graph
...
def BuildScan(scan, param, files, color, yvals, ycut):
    ...
    for i in range(graph.GetN()):
        print(graph.GetY()[i])
        if graph.GetY()[i] == 0:
            minvalue = graph.GetY()[i]
            minindex = i
            bestfit = graph.GetX()[i]

    print(bestfit)
    f = open("plotOutput.txt", "a")
    f.write(str(bestfit) + "\n")
    f.close()
...
parser.add_argument('--POI', help='use this parameter of interest', default='realHiggsMass')
